    ...
    update(t, dt) {
        for (const entity of this.scene) {
            if (entity.customProperties?.isDynamic) {
                // Iteratively resolve collisions to handle corners properly
                let iterations = 0;
                ...
                while (hadCollision && iterations < maxIterations) {
                    hadCollision = false;
                    let maxCorrection = 0;
                    let bestCorrection = [0, 0, 0];
                    
                    // Find the largest collision and resolve it
                    for (const other of this.scene) {
                        if (entity !== other && other.customProperties?.isStatic) {
                            ...
                            if (this.aabbIntersection(aBox, bBox)) {
                                const correction = this.calculateCorrectionVector(aBox, bBox);
                                const correctionMagnitude = vec3.length(correction);
                                
                                if (correctionMagnitude > maxCorrection) {
                                    maxCorrection = correctionMagnitude;
                    ...
                    // Apply only the largest correction per iteration
                    if (hadCollision) {
                        const transform = entity.getComponentOfType(Transform);
                        if (transform) {
                            vec3.add(transform.translation, transform.translation, bestCorrection);
                        }
                    }
